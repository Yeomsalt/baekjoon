/*
 * 10798번
 * Q. 아직 글을 모르는 영석이가 벽에 걸린 칠판에 자석이 붙어있는 글자들을 붙이는 장난감을 가지고 놀고 있다.
...
public class Test03 {
	public static void main(String[]args)throws IOException{
		...
		char [][] ch = new char[5][15];//5줄의 문장 최대 15글자
		
		for(int i=0; i<5; i++) {
			String str = br.readLine();
			for(int j=0; j<str.length(); j++) {//문자열의 길이만큼(문자열의 길이가 각 문자열마다 다르니까
				ch[i][j] = str.charAt(j);
			}
		}
		...
		for(int i=0; i<15; i++) {
			for(int j=0; j<5; j++) {
				if(ch[j][i] ==' '|| ch[j][i]=='\0' ) {//문자열에 빈칸이면(' ', 0, '\0') 다음으로 이동
					continue;
				}
